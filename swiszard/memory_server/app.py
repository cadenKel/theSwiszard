...
from .pm_routes import register_pm_routes
...
def _ensure_pm():
    ...
    with _PM_INIT_LOCK:
        ...
        try:
            _pm.migrate_schema_defaults(_get_conn())
        except Exception as exc:
            log.warning("pm migrate failed: %s", exc)
        _PM_INIT_DONE = True

# Register all PM routes
register_pm_routes(app, _ensure_pm, _get_conn, _pm, pm_backup)
